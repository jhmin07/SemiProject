	...
	$(document).ready(function(){
		...
		$("img#idcheck").click(function(){
			...
			$.ajax({
				...
				type:"post", // "GET" 또는 "POST"를 지정할 때는 method 가 아니라 type 이다! 또한, 대소문자 구분하지 않는다.
							 // type 을 생략하면 "GET" 이다.
				data:{"userid":$("input#userid").val()},			
				dataType:"json", 
				success:function(json){
					<%-- json 은 <%= ctxPath%>/member/idDuplicateCheck.up 을 했을 때 웹브라우저에서 보여지는 결과물을 자바스크립트 객체(object)로 되어진 것이다. --%>
